...
function () {
  ...
  _createClass(RestfulModelCollection, [{
    ...
    value: function _delete(itemOrId) {
      ...
      var item = itemOrId.id ? itemOrId : this.build({
        id: itemOrId
      });
    ...
    value: function deleteItem(options) {
      ...
      var body = options.hasOwnProperty('body') ? options.body : item.deleteRequestBody({});
      var qs = options.hasOwnProperty('qs') ? options.qs : item.deleteRequestQueryString({});
      return this.connection.request({
        method: 'DELETE',
        qs: qs,
        body: body,
        path: "".concat(this.path(), "/").concat(item.id)
      }).then(function () {
        if (callback) {
          callback(null);
        }
